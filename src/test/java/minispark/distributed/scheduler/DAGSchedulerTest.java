...
import minispark.distributed.worker.BasePartition;
...
class DAGSchedulerTest {
    ...
    private static class TestTaskScheduler implements TaskScheduler {
            ...
            for (int i = 0; i < tasks.size(); i++) {
                try {
                    ...
                    if (task instanceof RDDTask) {
                        ...
                        MiniRDD<?> rdd = getRddFromTask(rddTask);
        ...
        @SuppressWarnings("unchecked")
        private <T> MiniRDD<T> getRddFromTask(RDDTask<?> task) {
            try {
                // Use reflection to access the private rdd field
                java.lang.reflect.Field rddField = RDDTask.class.getDeclaredField("rdd");
