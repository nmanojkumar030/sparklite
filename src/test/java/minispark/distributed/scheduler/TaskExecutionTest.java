...
import minispark.distributed.worker.Worker;
...
class TaskExecutionTest {
    ...
    void shouldExecuteTasksAndReturnResults() throws Exception {
        ...
        EventLoopRunner.runUntil(messageBus, () -> futures.get(0).isDone(), java.time.Duration.ofSeconds(5));
        EventLoopRunner.runUntil(messageBus, () -> futures.get(1).isDone(), java.time.Duration.ofSeconds(5));
     ...
     */
    private static class TestTask extends Task<Void, Integer> {
