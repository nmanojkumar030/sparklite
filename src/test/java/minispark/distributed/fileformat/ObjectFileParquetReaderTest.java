...
import minispark.distributed.objectstore.Server;
import minispark.distributed.objectstore.LocalStorageNode;
...
class ObjectFileParquetReaderTest {
    ...
    void shouldEfficientlyReadFooterFromObjectStore(@TempDir java.nio.file.Path tempDir) throws IOException {
        ...
        try {
            ...
            EventLoopRunner.runUntil(messageBus, () -> partitionsFuture.isDone(), java.time.Duration.ofSeconds(10));
    ...
    private File createAndUploadParquetFile(java.nio.file.Path tempDir, String objectStoreKey, 
        ...
        try {
            ...
            EventLoopRunner.runUntil(messageBus, () -> putFuture.isDone(), java.time.Duration.ofSeconds(10));
            putFuture.get();
